...
void BgpRouter::processChunks(const BgpHeader& ptrHdr)
{
    switch (ptrHdr.getType()) {
		...
		//notification process add by xxl
        case BGP_NOTIFICATION:
            processMessage(*check_and_cast<const BgpNotificationMessage *>(&ptrHdr));
            break;
...
{
    BgpSession *session = _BGPSessions[_currSessionId];
    EV_INFO << "Processing BGP Notification message from " <<
            session->getPeerAddr().str(false) <<
            " with contents: \n";
    //printNotificationMessage(msg);
...
// The operation to delete a routing table entries
//  add by lja
bool BgpRouter::doDeleteBGPRoutingEntry(unsigned short peerAS){
        ...
        if (isDelete) {
            EV_INFO << "delete bgpRoutingTable contain " << peerAS << "\n";
        }
        else
            EV_INFO << "don't delete bgpRoutingTable contain " << peerAS << "\n";
        return isDelete;
}
